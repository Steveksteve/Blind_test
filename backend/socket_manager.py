...
# Dictionnaire pour suivre les utilisateurs dans les rooms
active_rooms = {}
...
def init_socketio(app):
    ...
    def handle_disconnect():
        ...
        if user:
            handle_leave_room({"room": user.room_id, "username": user.username})
    ...
    def handle_leave_room(data):
        ...
        if room_id in active_rooms and username in active_rooms[room_id]:
            ...
            if not active_rooms[room_id]:  # Si plus personne dans la room, suppression
                Room.query.filter_by(id=room_id).delete()
                db.session.commit()
                del active_rooms[room_id]
                print(f"🗑️ Room {room_id} supprimée car vide.")
